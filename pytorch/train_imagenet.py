...
from jittor.model import NetworkImageNet as Network
...
parser.add_argument('--save', type=str, default='DOTS_ImageNet_8471', help='experiment name')
parser.add_argument('--seed', type=int, default=random.randint(1,1000), help='random seed')
parser.add_argument('--arch', type=str, default='DOTS_ImageNet_8471', help='which architecture to use')
parser.add_argument('--grad_clip', type=float, default=5., help='gradient clipping')
parser.add_argument('--label_smooth', type=float, default=0.1, help='label smoothing')
parser.add_argument('--lr_scheduler', type=str, default='cosine', help='lr scheduler, linear or cosine')
parser.add_argument('--data_dir', type=str, default='/home/gyc/datasets/ImageNet2012', help='temp data dir')
parser.add_argument('--note', type=str, default='try', help='note for this run')


args, unparsed = parser.parse_known_args()

args.save = 'logs/eval/ImageNet-eval-{}-{}'.format(args.save, args.seed)
utils.create_exp_dir(args.save, scripts_to_save=glob.glob('*.py'))

log_format = '%(asctime)s %(message)s'
logging.basicConfig(stream=sys.stdout, level=logging.INFO,
    format=log_format, datefmt='%m/%d %I:%M:%S %p')
fh = logging.FileHandler(os.path.join(args.save, 'darts.txt'))
...
def main():
    ...
    flops, _ = profile(model, inputs=(torch.randn(1, 3, 224, 224),), verbose=False)
    ...
    logging.info('# images to train: %d' % num_train)
    logging.info('# images to validate: %d' % num_val)
